...
library(viridis)
```
#option one is using genomc annotations from txdb
loading packages
```{r}
...
```{r}
#example
readRDS("../data/processed/bins/CpGs/1kb/bottom1%/GSM1382253.rds")
```
We are going to create a genomic ranges object for each one, and we are going to annotate it based on overlaps.
But before doing that we need to "clean" the genomic ranges we are going to use for the annotation.
We want to use waterfall principle, and delete from the second annotation category the ranges that were already in the first category.
So when we annotate with exons we have to first delete the ranges that intersect with promoters (keeping only the ones that dont with setdiff)

##additionaly, (before deleting teh intersections) we want to (for this bulk type of annotation) to disjoin the ranges, to ensure that the length of overlap is nto artifitially inflated 
(we dont want to have two promoters for example overlapping with our tile in the same 50bp and our overlap to be 100 when in reality it is 50)

if we want to see which genes overlap later we have to change this
so we have to remove all the disjoins
```{r}
txdb<- TxDb.Mmusculus.UCSC.mm9.knownGene
#methods(class=class(txdb))
...
now finding the Annotation, the number of bases that overlap with each

find overlaps,find the length of overlap (width) using pintersect, 
save the queryHits and merge with the result of pintersect (the queryHit is the rownumber of the granges that we will use to find teh bin)
collapse the df, adding the overlaps that are for the same row
need to also add rows for the data that did not overlap (so add 0)

##make it for all samples
```{r}
...
#bina_long_1<-bina_long
#k<-rbind(bina_long_1,bina_long) #rbind
#k %>% ggplot(aes(x=sample_name, y=range, fill=region)) +geom_bar(stat="summary") +scale_y_continuous(label = scales::percent)

#k %>% ggplot(aes(x=sample_name, y=range, fill=region)) +geom_bar(stat="summary") +scale_y_continuous(labels = scales::label_percent(accuracy = 1))

#this
#k %>% ggplot(aes(x=sample_name, y=range, fill=region)) +geom_bar(stat="summary") +theme_classic()+theme(axis.text.x = element_text(angle = 90, hjust = 1))

#geom_text(aes(label = scales::percent(region)),
...
plot
```{r}
tobind %>% ggplot(aes(x=sample_name, y=range, fill=region)) +geom_bar(stat="summary") +theme_classic()
```
...
plot<-tobind2 %>% ggplot(aes(x=sample_name, y=range, fill=region)) +geom_bar(stat="summary") + labs(x="", y="", fill="", title = "Genomic Annotation of Top 1% ranked 1kb tiles") +theme_classic()+theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 7)) + scale_fill_viridis_d()
ggsave("../figures/genomic_Annotation_of_bins/1kb_windows/unstranded_bins/Top1_percent.png", plot = plot)
  plot
...
Exactly the same for the bottom 1 and top bottom 10%
...
plot<-tobind2 %>% ggplot(aes(x=sample_name, y=range, fill=region)) +geom_bar(stat="summary") + labs(x="", y="", fill="", title = "Genomic Annotation of Bottom 10% ranked 1kb tiles") +theme_classic()+theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 7)) + scale_fill_viridis_d()
ggsave("../figures/genomic_Annotation_of_bins/1kb_windows/unstranded_bins/Bottom10_percent.png", plot = plot)
  plot
...
```{r}
sample_names<-c("GSM4594635","GSM1382253","GSM2533056","GSM4837324","GSM1382256","GSM1545748") #modified
addback<-c("DNMT3A2", "DNMT3A2", "DNMT3A1", "DNMT3B1","DNMT3B1","DNMT3B1")
tobind<-data.frame() #used to rbind all dfs together
for(i in 1:6){
...
plot<-tobind2 %>% ggplot(aes(x=sample_name, y=range, fill=region)) +geom_bar(stat="summary") + labs(x="", y="", fill="", title = "Genomic Annotation of Top 1% ranked 1kb tiles") +theme_classic()+theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 7)) + scale_fill_viridis_d()
ggsave("../figures/genomic_Annotation_of_bins/1kb_windows/stranded_bins/top1_percent.png", plot = plot)
  plot
```
...
plot<-tobind2 %>% ggplot(aes(x=sample_name, y=range, fill=region)) +geom_bar(stat="summary") + labs(x="", y="", fill="", title = "Genomic Annotation of Bottom 1% ranked 1kb tiles") +theme_classic()+theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 7)) + scale_fill_viridis_d()
ggsave("../figures/genomic_Annotation_of_bins/1kb_windows/stranded_bins/bottom1_percent.png", plot = plot)
  plot
```